"""
runner_fmnist.py — FashionMNIST experiment runner (FedAvg vs HedonicMFG).
...
Usage:
    ...
    python runner_fmnist.py --alpha 0.3 --algos hedonicmfg
    python runner_fmnist.py --alpha 0.3 --algos fedavg

    # Quick smoke test:
    python runner_fmnist.py --alpha 0.3 --quick

    # After all alphas done, combine plots:
    python combine_plots.py --datasets fmnist
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="FL Runner — FashionMNIST (per-alpha)")
...
def main():
    ...
    print(f"\n[Runner] Loading FashionMNIST data (α={args.alpha}) ...")
    ...
    # Print summary
    print(f"\n{'='*65}")
    print(f"  RESULTS — FashionMNIST  α={args.alpha}")
    print(f"{'='*65}")
    ...
    if not args.no_plots and all_results:
        generate_run_plots(all_results, "fmnist", args.alpha, run_dir)

    print(f"\n[Done] α={args.alpha} results → {run_dir}/")
    print(f"       Run 'python combine_plots.py --datasets fmnist' when all alphas are done.")
