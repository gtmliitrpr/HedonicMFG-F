"""algorithms/fmnist/fedavg_fmnist.py — FedAvg for FashionMNIST"""
import time
...
def run_fedavg_fmnist(config, client_train_loaders, client_val_loaders,
                      global_test_loader, device):
    print("\n" + "="*50 + "\n  Running: FedAvg [FashionMNIST]\n" + "="*50)
    ...
    def model_copy():
        m = get_fmnist_model(config).to(device)
    ...
    for rnd in range(1, config["total_rounds"] + 1):
        ...
        for i in range(N):
            ...
            lm = local_train_fmnist_standard(
