...
def fast_model_copy(src: nn.Module, factory_fn, device) -> nn.Module:
    """
    Fast model copy via state_dict — avoids deepcopy on GPU tensors.
    factory_fn: callable that returns a fresh model instance (e.g. get_model(config))
    ~10-20x faster than copy.deepcopy on CUDA models.
    """
    dst = factory_fn().to(device)
    dst.load_state_dict(src.state_dict())
    return dst


# ──────────────────────────────────────────
# Federated Aggregation  — FAST VERSION
# ──────────────────────────────────────────
def fedavg_aggregate(models: list, weights: list) -> list:
    """
    Weighted FedAvg aggregation.
    models  : list of nn.Module (any device)
    weights : list of floats (normalised internally)
    Returns : list of aggregated CPU tensors — caller copies to model.
    Key fix: accumulates on CPU with in-place add_, no clone() calls.
    """
    assert len(models) == len(weights), "models and weights must match"
    total        = sum(weights)
    norm_weights = [w / total for w in weights]

    # Allocate on CPU — aggregation never needs GPU
    agg_params = [torch.zeros_like(p.data.cpu())
                  for p in models[0].parameters()]
    for model, w in zip(models, norm_weights):
        for agg_p, p in zip(agg_params, model.parameters()):
            agg_p.data.add_(p.data.cpu(), alpha=w)   # in-place, no clone
    return agg_params
...
def fedavg_aggregate_params(param_list: list, weights: list) -> list:
    ...
    agg    = [torch.zeros_like(p.cpu()) for p in param_list[0]]
    for params, w in zip(param_list, norm_w):
        for a, p in zip(agg, params):
            a.data.add_(p.data.cpu(), alpha=w)
    return agg
    ...
    for i in range(N):
        for j in range(N):
            sim_matrix[i, j] = cosine_similarity(
                grad_vectors[i], grad_vectors[j]).item()
    return sim_matrix
...
class ResultsTracker:
    def __init__(self, algorithm_name: str):
        ...
        self.global_accs     = []
        self.avg_client_accs = []
        self.round_times     = []
        self.metadata        = {}

    def log(self, rnd: int, global_acc: float,
            client_accs: list, elapsed: float = 0.0):
        self.global_accs.append(global_acc)
        self.avg_client_accs.append(
            np.mean(client_accs) if client_accs else 0.0)
