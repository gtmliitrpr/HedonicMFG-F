...
def run_hedonic_mfg_fmnist(config, client_train_loaders, client_val_loaders,
    ...
    def new_model():
        return get_fmnist_model(config).to(device)
    ...
    def new_head():
        return get_fmnist_personal_head(config).to(device)
    ...
    def apply_agg(model, agg_params):
        """Copy CPU agg tensors into GPU model in-place."""
    ...
    for rnd in range(1, warmup + 1):
        local_models = []
        for i in range(N):
            lm = copy_model(global_model)            # fast state_dict copy
    ...
    # Weight vectors on CPU — similarity computation stays off GPU
    client_model_weights = {
    ...
    personal_heads = {i: new_head() for i in range(N)} if use_ph else {}
    ...
    for rnd in range(warmup + 1, total_rounds + 1):
        ...
        for i in range(N):
            k  = c2k[i]
            lm = copy_model(coalition_models[k])     # fast state_dict copy
            opt_e, opt_tau = all_strats.get(i, (config["local_epochs_base"], 1.0))

            # Personal head: fast state_dict copy
            ph_copy = copy_head(personal_heads[i]) if (use_ph and i in personal_heads) else None
