"""
visualize.py — Per-run plots for K ablation study (MNIST).
generate_run_plots() called by runner for each K individually.
"""
...
def generate_run_plots(results: dict, dataset: str,
    ...
    """Generate convergence curve + bar chart for a single K run."""
    os.makedirs(save_dir, exist_ok=True)
    _plot_convergence(results, label, save_dir)
    _plot_bar(results, label, save_dir)


def _plot_convergence(results, label, save_dir):
    fig, axes = plt.subplots(1, 2, figsize=(13, 5))
    fig.suptitle(f"FL Convergence — MNIST ({label})",
        ...
        for name, tracker in results.items():
            ...
            if name == "HedonicMFG":
                ax.fill_between(rounds,
                                [v - 0.3 for v in sm],
                                [v + 0.3 for v in sm],
                                color=ALGO_COLORS["HedonicMFG"], alpha=0.10)
    ...
    path = os.path.join(save_dir, f"mnist_{tag}_convergence.png")
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"[Plot] {path}")


def _plot_bar(results, label, save_dir):
    ...
    fig.suptitle(f"Best Accuracy — MNIST ({label})",
    ...
    for ax, accs, title in zip(axes,
        ...
        bars   = ax.bar(x, accs, 0.55, color=colors,
        ...
        ax.set_ylim(max(0, min(accs) - 5) if accs else 0,
                    min(100, ymax + 5))

    plt.tight_layout()
    tag  = label.replace("=", "").replace(" ", "_")
    path = os.path.join(save_dir, f"mnist_{tag}_bar.png")
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"[Plot] {path}")
