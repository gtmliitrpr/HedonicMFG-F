"""algorithms/mnist/fedavg_mnist.py — FedAvg for MNIST"""
import time
from utils import ResultsTracker, print_round_summary, evaluate_model, fedavg_aggregate
from local_trainer_mnist import local_train_standard
from models_mnist import get_model


def run_fedavg_mnist(config, client_train_loaders, client_val_loaders,
                     global_test_loader, device):
    print("\n" + "="*50 + "\n  Running: FedAvg [MNIST]\n" + "="*50)
    ...
    # Factory for fast model copies — avoids deepcopy on GPU
    def model_copy():
    ...
    for rnd in range(1, config["total_rounds"] + 1):
        ...
        for i in range(N):
            lm = model_copy()                        # fast state_dict copy
        ...
        # Aggregate on CPU, copy back to GPU global model
        ...
        if rnd % 10 == 0 or rnd == 1:
            print_round_summary("FedAvg", rnd, config["total_rounds"],
                                 g_acc, sum(c_accs) / N)
