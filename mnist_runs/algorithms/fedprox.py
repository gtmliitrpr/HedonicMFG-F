"""
algorithms/fedprox.py — FedProx baseline.
From: Li et al. 2020, 'Federated Optimization in Heterogeneous Networks'
Adds proximal regularization term μ/2||w - w_t||² to local objective.
"""
...
from local_trainer import local_train_fedprox
from models import get_model


def run_fedprox(config, client_train_loaders, client_val_loaders,
    ...
    for rnd in range(1, total_rounds + 1):
        ...
        for i in range(num_clients):
            ...
            local_model = local_train_fedprox(
        ...
        if rnd % 10 == 0 or rnd == 1:
            print_round_summary("FedProx", rnd, total_rounds,
                                 global_acc, sum(client_accs)/len(client_accs))

    print(f"  [FedProx] Best global: {max(tracker.global_accs)*100:.2f}%  "
