"""
algorithms/imdb/hedonic_mfg_imdb.py — HedonicMFG for IMDB Sentiment

Key design for maximum advantage on IMDB:
  1. Weight-based hedonic utility (not gradient-based) — stable for NLP
  2. Individual client model tracking — coalition model = aggregate of members
  3. Fast MFG with coarse grid — efficient for NLP training
  4. High λ_fair=0.7 — sentiment skew is severe across clients
  5. Personalized 2-layer head — captures client-specific sentiment vocabulary
  6. K=3 coalitions — sentiment naturally clusters (pos-dominant/neg-dominant/mixed)
  7. Adam optimizer throughout — better for embedding layers
"""
...
from local_trainer_imdb import local_train_imdb_hedonic
from models_imdb import get_imdb_model, get_imdb_personal_head
...
def compute_weight_similarity(weights_i: list, weights_j: list) -> float:
    ...
    sim = torch.dot(flat_i, flat_j) / (flat_i.norm() * flat_j.norm() + 1e-8)
    return sim.item()


def compute_hedonic_utility_imdb(client_id, coalition, client_model_weights,
                                  client_accs, config):
    if not coalition:
        return -float("inf")
...
def form_coalitions_imdb(num_clients, K, client_model_weights,
    ...
    for _ in range(nash_iters):
        ...
        for client in order:
            ...
            curr_u    = compute_hedonic_utility_imdb(client, curr_coal,
            ...
            for tgt_k in range(K):
                ...
                u = compute_hedonic_utility_imdb(client, tgt_coal,
                                                  client_model_weights,
                                                  client_accs, config)
                if u > best_u + 1e-6:
...
# ═══════════════════════════════════════════════════════════
# MFG — Fast coarse grid
# ═══════════════════════════════════════════════════════════

def compute_mfg_payoff_imdb(action, mean_field, base_acc,
                              coalition_min_acc, config):
    epochs, tau = action
    mean_e, mean_t = mean_field

    effort    = (epochs / 8.0) * tau
    est_acc   = min(1.0, base_acc * (1.0 + 0.06 * effort))
    ...
    contrib   = config["delta_contrib"] * (epochs / 16.0)

    return est_acc - comp_cost - part_cost - sync_pen + fairness + contrib


def solve_mfg_imdb(coalition, client_accs, config):
    epoch_space = [2, 3, 5, 7, 8]
    ...
    coal_min_acc = min(accs) if accs else 0.5
    ...
    for _ in range(config["mfg_iterations"]):
        ...
        for c in coalition:
            ...
            be, bt   = strategies[c]
            bu       = compute_mfg_payoff_imdb((be, bt), mf, base_acc,
                                                coal_min_acc, config)
            for e in epoch_space:
                for tau in tau_space:
                    u = compute_mfg_payoff_imdb((e, tau), mf, base_acc,
                                                 coal_min_acc, config)
                    if u > bu + 1e-6:
                        bu, be, bt = u, e, tau
            strategies[c] = (be, bt)
...
def run_hedonic_mfg_imdb(config, client_train_loaders, client_val_loaders,
                          global_test_loader, device, vocab_size):
    print("\n" + "="*50)
    print("  Running: HedonicMFG [IMDB — Improved]")
    print("="*50)
    ...
    coalitions = form_coalitions_imdb(
        N, K, client_model_weights, client_accs_dict, config)
    ...
    personal_heads = {i: get_imdb_personal_head(config).to(device)
    ...
    for rnd in range(warmup + 1, total_rounds + 1):
        ...
        if (rnd - warmup) > 1 and (rnd - warmup - 1) % R == 0:
            ...
            old = coalitions
            coalitions = form_coalitions_imdb(
                N, K, client_model_weights, client_accs_dict,
                config, prev_coalitions=old)
        ...
        for k, coal in enumerate(coalitions):
            ...
            all_strats.update(solve_mfg_imdb(coal, coal_accs, config))
        ...
        for i in range(N):
            ...
            trained, trained_head = local_train_imdb_hedonic(
                lm, client_train_loaders[i],
                epochs=opt_e,
                lr=config["lr"],
                weight_decay=config["weight_decay"],
        ...
        for i in range(N):
            ...
            if use_ph and i in personal_heads:
                ...
                acc = evaluate_model(client_models[i], client_val_loaders[i], device)
