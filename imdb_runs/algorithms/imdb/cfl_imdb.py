...
def run_cfl_imdb(config, client_train_loaders, client_val_loaders,
    ...
    cluster_models = {0: get_imdb_model(config, vocab_size).to(device)}
    ...
    for rnd in range(1, config["total_rounds"] + 1):
        ...
        for clist in list(clusters):
            ...
            for i in clist:
                ...
                all_updates[i] = torch.cat([(lp2-ip).flatten()
                                             for lp2, ip in zip(lp, init_p)])
            ...
            if rnd > 5 and len(clist) >= 2:
                ...
                if mean_sim < eps1 and (max(norms)-min(norms)) > eps2:
