...
def run_ifca_imdb(config, client_train_loaders, client_val_loaders,
    ...
    cluster_models = [get_imdb_model(config, vocab_size).to(device) for _ in range(K)]
    ...
    for rnd in range(1, config["total_rounds"] + 1):
        t0 = time.time()
        assignments = [int(np.argmin([est_loss(cluster_models[k], client_train_loaders[i])
        ...
        global_eval = get_imdb_model(config, vocab_size).to(device)
        agg = fedavg_aggregate(cluster_models,
                                [s/total_s if total_s > 0 else 1/K for s in cluster_sizes])
        for p, ap in zip(global_eval.parameters(), agg): p.data.copy_(ap)

        g_acc = evaluate_model(global_eval, global_test_loader, device)
        c_accs = [evaluate_model(cluster_models[assignments[i]],
                                  client_val_loaders[i], device) for i in range(N)]
