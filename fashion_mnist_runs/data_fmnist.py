...
Natural clustering signal for HedonicMFG:
  - Clothing group: {0,1,2,3,4,6} — tops, bottoms, outerwear
  - Footwear group: {5,7,9}       — sandal, sneaker, boot
  - Accessory group: {8}          — bag (mixed with others)
...
# ──────────────────────────────────────────
# Transforms — slightly stronger augmentation
# than MNIST since FashionMNIST is harder
# ──────────────────────────────────────────
...
def get_fmnist_client_loaders(config: dict) -> tuple:
    ...
    print(f"\n[Data] FashionMNIST | {num_clients} clients | Dirichlet α={alpha}")
    ...
    for i, indices in enumerate(client_indices):
        if len(indices) < 10:
            indices = np.random.choice(len(targets), 50, replace=False)
