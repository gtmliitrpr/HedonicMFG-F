...
def run_cfl_adult(config, client_train_loaders, client_val_loaders,
    ...
    cluster_models = {0: get_adult_model(config, feature_dim).to(device)}
    ...
    for rnd in range(1, config["total_rounds"] + 1):
        ...
        c_accs = [evaluate_model(cluster_models[c2c[i]], client_val_loaders[i], device) for i in range(N)]
