...
def run_ifca_fmnist(config, client_train_loaders, client_val_loaders,
                    global_test_loader, device):
    print("\n" + "="*50 + "\n  Running: IFCA [FashionMNIST]\n" + "="*50)
    ...
    cluster_models = [get_fmnist_model(config).to(device) for _ in range(K)]
