...
from local_trainer_fmnist import local_train_fmnist_pfedme
from models_fmnist import get_fmnist_model

def run_pfedme_fmnist(config, client_train_loaders, client_val_loaders,
                      global_test_loader, device):
    print("\n" + "="*50 + "\n  Running: pFedME [FashionMNIST]\n" + "="*50)
    ...
    for rnd in range(1, config["total_rounds"] + 1):
        ...
        for i in range(N):
            ...
            w_i, personal_models[i] = local_train_fmnist_pfedme(
                w_i, global_model, client_train_loaders[i],
                config["pfedme_local_steps"], config["lr"],
                config["momentum"], config["pfedme_beta"],
                config["pfedme_lambda"], device)
        ...
        c_accs = [evaluate_model(personal_models[i], client_val_loaders[i], device)
                  for i in range(N)]
