...
def run_fedavg_fmnist(config, client_train_loaders, client_val_loaders,
    ...
    for rnd in range(1, config["total_rounds"] + 1):
        ...
        if rnd % 10 == 0 or rnd == 1:
            print_round_summary("FedAvg", rnd, config["total_rounds"], g_acc, sum(c_accs)/N)

    print(f"  [FedAvg] Best global: {max(tracker.global_accs)*100:.2f}%  Best client: {max(tracker.avg_client_accs)*100:.2f}%")
    return tracker
