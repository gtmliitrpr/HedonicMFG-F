"""SCAFFOLD — FashionMNIST. Control variates on CPU."""
import torch, copy, time
from utils import ResultsTracker, print_round_summary, evaluate_model, fedavg_aggregate
from local_trainer_fmnist import local_train_fmnist_scaffold
from models_fmnist import get_fmnist_model

def run_scaffold_fmnist(config, client_train_loaders, client_val_loaders,
                        global_test_loader, device):
    print("\n" + "="*50 + "\n  Running: SCAFFOLD [FashionMNIST]\n" + "="*50)
    ...
    for rnd in range(1, config["total_rounds"] + 1):
        ...
        for i in range(N):
            ...
            lm, new_ci, c_delta = local_train_fmnist_scaffold(
        ...
        for idx, c_g in enumerate(c_global):
            c_g.data.add_(sum(c_deltas[i][idx] for i in range(N)) / N)
